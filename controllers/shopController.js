...
const createShop = async (req, res) => {
    try {
        const { name } =  req.body;
        if (!name) {
            return res.status(401).json({ status: "error", message: "Nom de la boutique non fourni." });
        }

        const existingShopName = await Shop.findOne({ where: { name } });
        if (existingShopName) {
            return res.status(409).json({ status: "error", message: "Une boutique avec ce nom existe déjà." });
        }
...
const updateShop = async (req, res) => {
    try {
        ...
        if (existingName) {
            return res.status(409).json({ status: "error", message: "Ce nom existe déjà dans la base de données." });
        }
        ...
        res.status(500).json({
            status: "error",
            message: "Une erreur s'est produite lors de la mise à jours de la boutique.",
        });
    }
};
...
const deleteShop = async (req, res) => {
    ...
    try {
        ...
        if (!shop) {
            return res.status(404).json({ status: 'error', message: 'Boutique non trouvé.' });
        }
        ...
        res.status(500).json({
            status: "error",
            message: "Une erreur s'est produite lors de la suppéssion de la boutique.",
        });
    }
};
...
module.exports = { createShop, updateShop, deleteShop, allShop };